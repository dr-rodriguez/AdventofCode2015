...
asking you to enter a code from the instruction manual. Apparently, it refuses to run unless
...
the codes are filled in diagonally up and to the right. This process repeats until the infinite
...
# Function to generate the code. Works only for low numbers as otherwise it exceeds the maximum recursion depth.
def gen_code(r,c):
    if r==1 and c==1:
        return 20151125
    else:
        ...
        return (gen_code(r_new, c_new) * 252533) % 33554393
...
#print 'Goal:', gen_code(r_goal, c_goal)
# This formulation exceeds the maximum recursion depth, but can be used to test for low numbers

while r!=r_goal or c!=c_goal:
    if r==1:
        r_new = c + 1
        c_new = 1
    else:
